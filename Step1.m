...
% By Yonghui Fan, last edited by Yuxiang Liu
% Matlab 2013b and later version is needed.
% iso2mesh, FreesurferMatlab and tetgen are needed

start_path = 'C:\ADNI\ADNI422_freesurfer'; % data directory
...
remove_id = []; % record all removed vertices
...
for n = 1:numberOfFolders
    ...
    for cnt = 1:3 % if after 3 times intersections still not solved, give up
        ...
        remove_wv = ia;
        remove_pv = ib;
        r_id = [remove_pv;remove_wv + size(pv2,1)]; % removed vertices in one iteration
        remove_id = union(remove_id, r_id);
        ...
        int_cnt = length(readIntersectionFile([dir,'\self_intersection.txt']));
        if int_cnt == 0 % check if all intersections are solved
            break
        end
    ...
    %record = fopen('E:\lyx\tetgen\record422_int.txt', 'At');
